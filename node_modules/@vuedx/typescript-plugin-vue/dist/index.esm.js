import { ConfiguredVueProject, InferredVueProject, createFullAnalyzer, transformToFunction } from '@vuedx/analyze';
import Path from 'path';
import { VueTextDocument, isVirtualFile, getContainingFile, DocumentStore, asUri, isVueFile, MODULE_SELECTOR, parseVirtualFileName, VIRTUAL_FILENAME_SEPARATOR, INTERNAL_MODULE_SELECTOR, RENDER_SELECTOR, SCRIPT_BLOCK_SELECTOR, SCRIPT_SETUP_BLOCK_SELECTOR, isVirtualFileOfType, VirtualTextDocument } from '@vuedx/vue-virtual-textdocument';
import JSON5 from 'json5';
import { URI } from 'vscode-uri';
import { findTemplateNodeAt, findTemplateNodeInRange, findTemplateChildrenInRange, isSimpleIdentifier, isElementNode, isDirectiveNode, traverseFast, isComponentNode, isSimpleExpressionNode, createSimpleExpression, stringify, isTextNode, isAttributeNode, isPlainElementNode, findParentNode as findParentNode$1, traverseEvery } from '@vuedx/template-ast-types';
...
class ProxyDocumentStore extends DocumentStore {
    get(fileNameOrUri) {
...
class PluginContext {
    ...
    getVueProjectForFile(fileName, ensure) {
        ...
        if (project === null && ensure === true) {
            ...
            const rootDir = Path.posix.dirname((_b = packageFile !== null && packageFile !== void 0 ? packageFile : configFile) !== null && _b !== void 0 ? _b : fileName);
            ...
            const newProject = configFile != null
                ? new ConfiguredVueProject(rootDir, packageFile, packageFile != null ? tryRequire(packageFile) : {}, configFile, tryRequire(configFile))
                : new InferredVueProject(rootDir, packageFile, packageFile != null ? tryRequire(packageFile) : {});
...
function patchProject(context, project) {
    const vue2supportFile = context.typescript.server.toNormalizedPath(Path.join(Path.dirname(__dirname), 'runtime/vue-2.d.ts'));
    tryPatchMethod(project, 'getFileNames', (getFileNames) => (excludeFilesFromExternalLibraries, excludeConfigFiles) => {
        const fileNames = new Set(getFileNames(excludeFilesFromExternalLibraries, excludeConfigFiles).map((fileName) => isVirtualFile(fileName) ? getContainingFile(fileName) : fileName));
                ...
                if (scriptInfo != null && document != null) {
                    if (!scriptInfo.isScriptOpen()) {
                        ...
                        if (version > document.version) {
                            ...
                            VueTextDocument.update(document, [{ text: snapshot.getText(0, snapshot.getLength()) }], version);
                        }
                    }
                }
...
function patchModuleResolution(context, languageServiceHost) {
    ...
    tryPatchMethod(languageServiceHost, 'resolveModuleNames', (resolveModuleNames) => {
        ...
        return wrapFn('resolveModuleNames', (moduleNames, containingFile, reusedNames, redirectedReferences, options) => {
            ...
            const newModuleNames = moduleNames.map((moduleName) => isVueFile(moduleName)
...
function patchWatchFile(context) {
    tryPatchMethod(context.serviceHost, 'watchFile', (watchFile) => {
        ...
        return wrapFn('watchFile', (fileName, callback, pollingInterval, options) => {
            if (isVirtualFile(fileName)) {
                return watchFile(getContainingFile(fileName), (_, event) => {
...
function patchScriptInfo(context, scriptInfo) {
    ...
    tryPatchMethod(scriptInfo, 'editContent', (editContent) => {
        ...
        return wrapFn('editContent', (start, end, newText) => {
            ...
            const newDocument = VueTextDocument.update(document, [{ range, text: newText }], getLastNumberFromVersion(scriptInfo.getLatestVersion()));
...
function createServerHelper(context, languageService) {
    ...
    function getDocument(fileName) {
        ...
        return isVirtualFile(fileName)
            ? (_b = (_a = context.store.get(getContainingFile(fileName))) === null || _a === void 0 ? void 0 : _a.getDocument(fileName)) !== null && _b !== void 0 ? _b : null : context.store.get(fileName);
    }
    ...
    function getAllDocuments(fileName) {
        ...
        if (document != null) {
            if (isVirtualFile(fileName)) {
                ...
                documents.push(document.getDocument(RENDER_SELECTOR), document.getDocument(INTERNAL_MODULE_SELECTOR), document.getDocument(MODULE_SELECTOR), document.getDocument(SCRIPT_BLOCK_SELECTOR), document.getDocument(SCRIPT_SETUP_BLOCK_SELECTOR));
            }
        }
    ...
    function isRenderFunctionDocument(document) {
        var _a;
        return ((_a = document) === null || _a === void 0 ? void 0 : _a.selector.type) === RENDER_SELECTOR;
    }
    function isRenderFunctionFileName(fileName) {
        var _a;
        return (isVirtualFile(fileName) &&
            ((_a = parseVirtualFileName(fileName)) === null || _a === void 0 ? void 0 : _a.selector.type) === RENDER_SELECTOR);
    }
...
const RefactorExtractComponent = {
    ...
    applyRefactor(config, fileName, _, position, refactorName, actionName) {
        ...
        if (conditionType === 'preserve') {
            nodes.forEach((node) => {
                if (isElementNode(node)) {
                    addSkipNodes(node.props.filter((prop) => isDirectiveNode(prop) && /^(if|else-if|else)$/.test(prop.name)), [...node.props, ...node.children]);
                }
            });
        }
        ...
        if (nodes.length === 1) {
            ...
            if (isElementNode(el)) {
                const forNode = el.props.find((prop) => isDirectiveNode(prop) && prop.name === 'for');
                if (forNode != null) {
        ...
        const emits = new Map();
        nodes.forEach((node) => traverseFast(node, (node) => {
                ...
                if (isSimpleExpressionNode(node.exp)) {
                    if (node.name === 'model') {
                        ...
                        if (isSimpleIdentifier(node.exp.content.trim())) {
                            ...
                            replaceNodes.set(node.exp, createSimpleExpression(`$emit('${name}')`, false));
                            emits.set(name, name);
                        }
                    }
                }
        ...
        sfc.push(`<template>${stringify(nodes, {
            indent: 2,
            initialIndent: 1,
...
function genComponentImport(oldFileName, newFileName, source) {
    ...
    if (moduleName.startsWith('.')) {
        moduleName = Path.posix.relative(Path.posix.dirname(newFileName), Path.posix.resolve(Path.posix.dirname(oldFileName), moduleName));
        if (!moduleName.startsWith('.')) {
...
function detectConditionType(parent, nodes) {
    const conditions = nodes.map((node) => {
        if (isElementNode(node)) {
            return node.props.find((prop) => isDirectiveNode(prop) && /^(if|else-if|else)$/.test(prop.name));
        }
    });
    ...
    if (nonNullConditions.length > 0) {
        ...
        const nextCondition = isElementNode(nextSibling)
            ? nextSibling.props.find((prop) => isDirectiveNode(prop) && /^(else-if|else)$/.test(prop.name))
            : undefined;
...
function getImportEditForComponent(document, info, fileName, project) {
    ...
    const relativeFileName = `./${Path.posix.relative(Path.posix.dirname(document.fsPath), fileName)}`;
...
const RefactorExtractComputed = {
    ...
    findRefactors({ helpers, service }, fileName, position, preferences) {
        ...
        if (document != null && isSimpleExpressionNode(node)) {
            ...
            else if (isDirectiveNode(parent) && // Unsupported directives.
                (parent.name === 'slot' ||
                    parent.name === 'for' ||
                    parent.name === 'on' ||
                    parent.name === 'model')) {
...
const RefactorExtractMethod = {
    ...
    applyRefactor({ helpers, context, service }, fileName, _, position, refactorName, action) {
        ...
        if (isSimpleExpressionNode(node) && document !== null) {
            ...
            const isEventHandler = isDirectiveNode(parent) && parent.name === 'on' && parent.exp === node;
...
class HTMLService {
    ...
    getAttributeCompletionsAtPosition(document, _position, options, element, attribute) {
        ...
        if (isAttribute || isEvent) {
            const alreadyExists = new Set(element.props
                .map((prop) => isAttributeNode(prop) && !/^(class|style)$/.test(prop.name) // TODO: Handle static/dynamic class and style attributes.
                ? prop.name
        ...
        if (!isDirectiveNode(attribute)) {
            ...
            if (isComponentNode(element)) {
                ...
                const parent = findParentNode$1(document.ast, element);
    ...
    getAttributeCompletionEntryDetails(document, _position, entryName, formatOptions, source, preferences, element, attribute) {
        ...
        const isAttribute = (isDirectiveNode(attribute) && attribute.name === 'bind') ||
...
const GotoExternalComponentProp = {
    ...
    getDefinitionAndBoundSpan({ helpers: h, context }, fileName, position) {
        ...
        let name;
        let node;
        if (isDirectiveNode(targetNode === null || targetNode === void 0 ? void 0 : targetNode.node) &&
        ...
        else if (isAttributeNode(targetNode === null || targetNode === void 0 ? void 0 : targetNode.node)) {
        ...
        if (resolvedModule == null)
            return;
        if (!isVirtualFile(resolvedModule.resolvedFileName))
            return;
        const externalComponentFilename = getContainingFile(resolvedModule.resolvedFileName);
...
const RenameElementTag = {
    ...
    canRename(config, fileName, position) {
        ...
        if (isPlainElementNode(node) && isPositionInTagName$1(position, node)) {
            return {
                canRename: true,
                displayName: node.tag,
                fullDisplayName: node.tag,
...
function createTemplateLanguageServer(config) {
    ...
    return wrapInTrace('TemplateLanguageServer', {
        ...
        getJsxClosingTagAtPosition(fileName, position) {
            const { node } = h.findTemplateNodeAtPosition(getContainingFile(fileName), position);
            if (isElementNode(node)) {
    ...
    return wrapInTrace('VueLanguageServer', {
                                                        ...
                                                        traverseEvery(render.ast, (node) => {
                                                            ...
                                                            if (isSimpleExpressionNode(node) &&
                                                                ...
                                                                if (binding == null) {
                                                                    ...
                                                                    const additionalLocations = template.findRenameLocations(document.container.getDocumentFileName(RENDER_SELECTOR), node.loc.start.offset +
        ...
        getEditsForFileRename(oldFilePath, newFilePath, formatOptions, preferences) {
            ...
            if (document != null) {
                ...
                const currentChanges = script.getEditsForFileRename(component.fsPath, isVueFile(newFilePath)
...
function createLanguageServiceRouter(config) {
    ...
    const VIRTUAL_FILE_SUFFIX_RE = new RegExp(`(?<=\\.vue)${VIRTUAL_FILENAME_SEPARATOR}([A-Za-z_][A-Za-z0-9_-]*)(\\.[jt]sx?)?`, 'g');
    function getComponentName$1(fileName) {
        if (fileName == null || !isVueFile(fileName))
            return;
    ...
    const proxy = wrapInTrace('LanguageRoutingService', {
        ...
        getCompletionEntryDetails(fileName, position, entryName, formatOptions, source, preferences) {
            ...
            (_b = details === null || details === void 0 ? void 0 : details.codeActions) === null || _b === void 0 ? void 0 : _b.forEach((codeAction) => {
                codeAction.description = codeAction.description.replace(VIRTUAL_FILENAME_SEPARATOR + MODULE_SELECTOR, '');
                codeAction.changes.forEach((change) => {
                    change.textChanges.forEach((textChange) => {
                        textChange.newText = textChange.newText.replace(VIRTUAL_FILENAME_SEPARATOR + MODULE_SELECTOR, '');
                    });
        ...
        getCodeFixesAtPosition(fileName, start, end, errorCodes, formatOptions, preferences) {
            return choose(fileName)
                ...
                if (fix.fixName === 'import' &&
                    ...
                    return fix.description.includes(VIRTUAL_FILENAME_SEPARATOR + MODULE_SELECTOR);
                }
                return true;
            })
                .map((fix) => {
                if (fix.fixName === 'import') {
                    fix.description = fix.description.replace(VIRTUAL_FILENAME_SEPARATOR + MODULE_SELECTOR, '');
            ...
            if (diagnostic.relatedInformation != null) {
                diagnostic.relatedInformation = diagnostic.relatedInformation
                    ...
                    if (relatedInformation.file != null &&
                        ...
                        if (isVirtualFileOfType(relatedInformation.file.fileName, MODULE_SELECTOR) ||
                            isVirtualFileOfType(relatedInformation.file.fileName, INTERNAL_MODULE_SELECTOR)) {
                            return null;
                        }
                        relatedInformation.file = (_a = config.helpers.getSourceFile(getContainingFile(relatedInformation.file.fileName), ts)) !== null && _a !== void 0 ? _a : Object.assign(Object.create(Object.getPrototypeOf(diagnostic.file)), diagnostic.file, {
