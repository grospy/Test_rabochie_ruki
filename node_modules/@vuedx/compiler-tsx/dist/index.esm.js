import { findDir, isSimpleIdentifier, createStructuralDirectiveTransform, createSimpleExpression, processFor, createCallExpression, RENDER_LIST, createCompoundExpression, createFunctionExpression, createForLoopParams, findProp, RENDER_SLOT, buildSlots, WITH_CTX, processIf, baseParse, transform, OPEN_BLOCK, CREATE_BLOCK, CREATE_VNODE, FRAGMENT, generate } from '@vue/compiler-core';
import { isInterpolationNode, isSimpleExpressionNode, isElementNode, isDirectiveNode, traverse as traverse$1, isComponentNode, isAttributeNode, isCommentNode, isTextNode, isNode, createSimpleExpression as createSimpleExpression$1 } from '@vuedx/template-ast-types';
import Path from 'path';
import { parse as parse$1, parseExpression } from '@babel/parser';
import { traverse, isIdentifier, isFunction, traverseFast, isObjectMember, isAssignmentPattern, isMemberExpression, isOptionalMemberExpression, isArrayPattern } from '@babel/types';
...
function createElementTransform(options, expressions) {
    ...
    return (node, context) => {
        ...
        return () => {
            ...
            let startTag = createSimpleExpression(name, false, createLoc(node.loc, node.loc.source.indexOf(node.tag), node.tag.length), false);
    ...
    return createStructuralDirectiveTransform(/^(if|else-if|else)$/, (node, dir, context) => {
        ...
        return processIf(node, dir, context, (ifNode, branch, isRoot) => {
            return () => {
                ...
                let hasElse = false;
                if (dir.name !== 'else') {
                    branch.condition = createSimpleExpression$1((_a = content === null || content === void 0 ? void 0 : content.trimEnd()) !== null && _a !== void 0 ? _a : 'false', false, exp === null || exp === void 0 ? void 0 : exp.loc);
                }
    ...
    transform(ast, {
        ...
        nodeTransforms: [
            ...
            (node, context) => {
                if (isCommentNode(node)) {
                    if (node.content.includes('<') || node.content.includes('>')) {
                        context.replaceNode(createCompoundExpression([transformText(node.content.trim())]));
                    }
                    else {
                    ...
                    context.replaceNode(createCompoundExpression([transformText(node.content)]));
                }
                else if (isElementNode(node) && node.tag.includes('<')) {
                    context.replaceNode(createCompoundExpression([transformText(node.loc.source.trim())]));
                }
            },
