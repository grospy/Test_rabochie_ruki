...
function createExpressionTracker(addIdentifer) {
    return (node, context) => {
        ...
        if (templateAstTypes.isInterpolationNode(node)) {
            if (templateAstTypes.isSimpleExpressionNode(node.content) &&
            ...
            node.props.forEach((dir) => {
                if (templateAstTypes.isDirectiveNode(dir)) {
...
asRawStatements = false) {
    ...
    rawExp = rawExp
        ...
        .replace(/(\.|\[\]?)$/, '');
    if (compilerCore.isSimpleIdentifier(rawExp)) {
        if (!asParams &&
        ...
        types.traverse(ast, {
            enter(node, ancestors) {
                ...
                if (types.isIdentifier(node)) {
                    ...
                    node.params.forEach((param) => types.traverseFast(param, (node, ancestors) => {
                        var _a;
                        const parent = (_a = ancestors[ancestors.length - 1]) === null || _a === void 0 ? void 0 : _a.node;
                        if (types.isIdentifier(node) &&
                            !isStaticProperty(parent) &&
                            !(types.isAssignmentPattern(parent) && parent.right === node)) {
                            const id = node.name;
                            if (!scope.has(id)) {
...
function createTransformFor(addIdentifer) {
    return compilerCore.createStructuralDirectiveTransform(/^for$/, (node, dir, context) => {
        ...
        return compilerCore.processFor(node, dir, context, (forNode) => {
            ...
            forNode.codegenNode = compilerCore.createCompoundExpression([renderExp]);
            return () => {
...
function getIdentifiers(source) {
    ...
    try {
        ...
        types.traverse(ast, (node, ancestors) => {
            if (types.isIdentifier(node)) {
...
function parseUsingBabel(source) {
    try {
...
function isForNode(node) {
    return templateAstTypes.isNode(node) && node.type === 11;
}
function isIfNode(node) {
    return templateAstTypes.isNode(node) && node.type === 9;
}
function createElementTransform(options, expressions) {
    ...
    const hoist = (exp) => {
        ...
        expressions.push(compilerCore.createCompoundExpression([`const `, name, ' = ', exp, ';']));
        return name;
    };
    return (node, context) => {
        var _a, _b;
        ...
        return () => {
            var _a, _b;
            if (node.tag === 'slot') {
                const slotName = compilerCore.findProp(node, 'name');
                const props = node.props.filter((prop) => prop !== slotName &&
                    (templateAstTypes.isAttributeNode(prop) || prop.name === 'bind'));
                const args = compilerCore.createCompoundExpression(props.length === 0
                    ? []
                    : props.length === 1 &&
                        templateAstTypes.isDirectiveNode(props[0]) &&
                        props[0].arg == null
                        ? [props[0].exp]
                        : [
                            '{',
                            props.flatMap((prop) => {
                                ...
                                if (templateAstTypes.isAttributeNode(prop)) {
                                    ...
                                    if (templateAstTypes.isSimpleExpressionNode(prop.arg)) {
                                        return [
                                            '[',
                                            prop.arg.isStatic
                                                ? prop.arg
                                                : compilerCore.createSimpleExpression(prop.arg.content, prop.arg.isStatic, createLoc(prop.arg.loc, 1, prop.arg.content.length)),
                                            ']:',
                ...
                node.codegenNode = compilerCore.createCompoundExpression([
                    '{',
                    context.helper(compilerCore.RENDER_SLOT),
                    '(',
                    '_ctx.$slots',
                    ',',
                    templateAstTypes.isAttributeNode(slotName)
                        ? compilerCore.createSimpleExpression((_b = (_a = slotName.value) === null || _a === void 0 ? void 0 : _a.content) !== null && _b !== void 0 ? _b : 'default', true, slotName.loc)
                        : templateAstTypes.isDirectiveNode(slotName)
                            ? slotName.exp
                            : compilerCore.createSimpleExpression('default', true),
                    ',',
                    args,
                    ')',
            ...
            let startTag = compilerCore.createSimpleExpression(name, false, createLoc(node.loc, node.loc.source.indexOf(node.tag), node.tag.length), false);
            if (node.tag === 'component') {
                const isProp = compilerCore.findProp(node, 'is');
                if (templateAstTypes.isAttributeNode(isProp) && isProp.value != null) {
                    name = hoist(compilerCore.createSimpleExpression(`${JSON.stringify(isProp.value.content)} as const`, false, isProp.value.loc));
                    startTag = compilerCore.createSimpleExpression(name, false, startTag.loc);
                }
                else if (templateAstTypes.isDirectiveNode(isProp) &&
                    templateAstTypes.isSimpleExpressionNode(isProp.exp)) {
                    name = hoist(isProp.exp);
                    startTag = compilerCore.createSimpleExpression(name, false, startTag.loc);
                }
                else {
                    name = '';
                    startTag = compilerCore.createSimpleExpression('', false);
                }
            }
            ...
            if (node.isSelfClosing) {
                ...
                const children = generateChildren(node, context, resolvedComponentName != null);
                node.codegenNode = compilerCore.createCompoundExpression([
                    '<',
                    startTag,
                    ' ',
...
function generateJSXAttributes(node, context) {
    ...
    const ignore = node.tag === 'component' ? compilerCore.findProp(node, 'is') : undefined;
    node.props.forEach((dir) => {
        if (dir === ignore)
            return;
...
function generateAttribute(attr, node, sep = '=') {
    const code = [];
    if (sep === '=' && (attr.name === 'class' || attr.name === 'style')) {
        ...
        if (attr.value != null) {
            code.push(sep, compilerCore.createSimpleExpression(attr.value.loc.source, false, attr.value.loc));
        }
        else if (sep === ':') {
            code.push(sep, 'true');
        }
...
function generateVOn(dir, node) {
    ...
    if (templateAstTypes.isSimpleExpressionNode(dir.arg)) {
        if (dir.arg.isStatic) {
            code.push(compilerCore.createSimpleExpression(camelCase('on-' + dir.arg.content), false, dir.arg.loc), '={', ...exp, '}');
        }
        else {
            code.push('{...({[', dir.arg, ']: ', ...exp, '})}');
        }
...
function generateVBind(dir, node) {
    ...
    if (templateAstTypes.isSimpleExpressionNode(dir.arg)) {
        if (dir.arg.isStatic || dir.arg.content === 'key') {
            ...
            code.push(' {...({[', compilerCore.createSimpleExpression(dir.arg.content, dir.arg.isStatic, createLoc(dir.arg.loc, 1, dir.arg.content.length)), ']: ');
            if (dir.exp != null)
                code.push(dir.exp);
            else
                code.push('true');
...
function generateChildren(node, context, isResolvedComponent) {
    if (isResolvedComponent) {
        ...
        if (isDynamicSlotsExpression(slots)) {
            slots.arguments[0].properties = slots.arguments[0].properties.filter((property) => !(templateAstTypes.isSimpleExpressionNode(property.key) && property.key.content === '_'));
        }
        else {
            slots.properties = slots.properties.filter((property) => !(templateAstTypes.isSimpleExpressionNode(property.key) && property.key.content === '_'));
        }
        return [compilerCore.createCompoundExpression(['{', slots, '}'])];
    }
    else {
    ...
    return nodes.flatMap((node) => {
        if (templateAstTypes.isCommentNode(node)) {
            if (node.content.includes('<') || node.content.includes('>')) {
                return compilerCore.createCompoundExpression([transformText(node.content)]);
            }
            else {
                return [];
            }
            ...
            return compilerCore.createCompoundExpression(['{', node, '}']);
        }
        else {
            return node;
        }
    ...
    return compilerCore.createStructuralDirectiveTransform(/^(if|else-if|else)$/, (node, dir, context) => {
        ...
        return compilerCore.processIf(node, dir, context, (ifNode, branch, isRoot) => {
            return () => {
                ...
                let hasElse = false;
                if (dir.name !== 'else') {
                    branch.condition = templateAstTypes.createSimpleExpression((_a = content === null || content === void 0 ? void 0 : content.trimEnd()) !== null && _a !== void 0 ? _a : 'false', false, exp === null || exp === void 0 ? void 0 : exp.loc);
                }
...
function createInterpolationTransform(options) {
    return function transform(node, context) {
        if (templateAstTypes.isInterpolationNode(node)) {
            context.replaceNode(compilerCore.createCompoundExpression(['{', node.content, '}']));
        }
    };
}
...
function compile(template, options) {
    ...
    compilerCore.transform(ast, {
        ...
        nodeTransforms: [
                ...
                if (templateAstTypes.isInterpolationNode(node)) {
                    ...
                    node.props.forEach((prop) => {
                        if (templateAstTypes.isDirectiveNode(prop)) {
                            ...
                            if (templateAstTypes.isSimpleExpressionNode(prop.arg) && !prop.arg.isStatic) {
                                expressions.push([
            ...
            (node, context) => {
                if (templateAstTypes.isCommentNode(node)) {
                    if (node.content.includes('<') || node.content.includes('>')) {
                        context.replaceNode(compilerCore.createCompoundExpression([transformText(node.content.trim())]));
                    }
                    else {
                    ...
                    context.replaceNode(compilerCore.createCompoundExpression([transformText(node.content)]));
                }
                else if (templateAstTypes.isElementNode(node) && node.tag.includes('<')) {
                    context.replaceNode(compilerCore.createCompoundExpression([transformText(node.loc.source.trim())]));
                }
            },
    ...
    const hasVSlot = ast.helpers.includes(compilerCore.RENDER_SLOT);
    [
        compilerCore.OPEN_BLOCK,
        compilerCore.CREATE_BLOCK,
        compilerCore.CREATE_VNODE,
        compilerCore.FRAGMENT,
        compilerCore.RENDER_LIST,
        compilerCore.RENDER_SLOT,
    ].forEach((helper) => {
    ...
    const mappings = [];
    const result = compilerCore.generate(ast, {
        ...options,
        sourceMap: true,
        mode: 'module',
                    ...
                    if (hoists.length > 0) {
                        ...
                        hoists.forEach((hoist) => {
                            hoist.children.forEach((child) => {
                                if (templateAstTypes.isSimpleExpressionNode(child)) {
