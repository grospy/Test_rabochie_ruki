...
const compilerTsx = require('@vuedx/compiler-tsx');
...
const traverse__default = /*#__PURE__*/_interopDefaultLegacy(traverse);
const T__default = /*#__PURE__*/_interopDefaultLegacy(T);
const generate__default = /*#__PURE__*/_interopDefaultLegacy(generate);
const micromatch__default = /*#__PURE__*/_interopDefaultLegacy(micromatch);
...
function createAnalyzer(plugins, options = {}) {
    ...
    function analyzeScript(content, fileName = 'anonymous.js', mode = 'script') {
        const ext = Path__default['default'].posix.extname(fileName);
...
function processScript(context) {
    ...
    traverse__default['default'](context.ast, {
        enter(path) {
...
const ScriptIdentifierSourceAnalyzer = createPlugin({
    babel(node$, context) {
        ...
        if (T__default['default'].isProgram(node)) {
            const register = (node, kind) => {
                if (T__default['default'].isIdentifier(node)) {
                    context.component.addIdentifier(node.name, `scriptSetup:${kind}`, createSourceRange(context, node));
                }
                else if (T__default['default'].isObjectPattern(node)) {
                    node.properties.forEach((property) => {
                        if (T__default['default'].isObjectProperty(property)) {
                ...
                else if (T__default['default'].isArrayPattern(node)) {
                    node.elements.forEach((element) => {
                        if (T__default['default'].isRestElement(element)) {
                            register(element.argument, kind);
                        }
                        else if (T__default['default'].isAssignmentPattern(element)) {
                ...
                else if (T__default['default'].isMemberExpression(node)) {
                    register(node.object, kind);
                }
            };
            const processStatement = (statement) => {
                if (T__default['default'].isLabeledStatement(statement)) {
                    if (statement.label.name === 'ref') {
                        if (T__default['default'].isExpressionStatement(statement.body)) {
                            if (T__default['default'].isAssignmentExpression(statement.body.expression)) {
                                register(statement.body.expression.left, 'refLabel');
                            }
                        }
                    }
                }
                else if (T__default['default'].isVariableDeclaration(statement)) {
                    statement.declarations.forEach(({ id }) => {
                        register(id, 'value');
                    });
                }
                else if (T__default['default'].isFunctionDeclaration(statement)) {
...
function setIdentifiersSource(node, source, context, allowedTypes = ['object']) {
    const register = (obj) => {
        if (allowedTypes.includes('object') && T__default['default'].isObjectExpression(obj)) {
            obj.properties.forEach((property) => {
                if (T__default['default'].isObjectMember(property)) {
                    if (T__default['default'].isIdentifier(property.key)) {
                        context.component.addIdentifier(property.key.name, source, createSourceRange(context, property.key));
                    }
                }
            });
        }
        else if (allowedTypes.includes('array') && T__default['default'].isArrayExpression(obj)) {
            obj.elements.forEach((element) => {
                if (T__default['default'].isStringLiteral(element)) {
                    context.component.addIdentifier(element.value, source, createSourceRange(context, element));
                }
            });
        }
    };
    if (T__default['default'].isObjectProperty(node)) {
        if (T__default['default'].isObjectExpression(node.value) || T__default['default'].isArrayExpression(node.value)) {
            register(node.value);
        }
        else if (T__default['default'].isFunctionExpression(node.value) ||
            T__default['default'].isArrowFunctionExpression(node.value)) {
...
function getReturnExpression(node) {
    var _a, _b;
    if (T__default['default'].isBlockStatement(node.body)) {
        return ((_b = (_a = node.body.body.find((node) => T__default['default'].isReturnStatement(node))) === null || _a === void 0 ? void 0 : _a.argument) !== null && _b !== void 0 ? _b : undefined);
    }
    else if (T__default['default'].isExpression(node.body)) {
        return node.body;
    }
}
    ...
    babel: (path$, ctx) => {
        ...
        if (path$.isExportNamedDeclaration()) {
            ...
            if (((_a = node.source) === null || _a === void 0 ? void 0 : _a.value.endsWith('.vue')) === true) {
                const specifier = node.specifiers.find((specifier) => T.isExportSpecifier(specifier) &&
            ...
            if (T.isObjectProperty(property)) {
                ...
                if (T.isObjectExpression(components)) {
                    components.properties.forEach((declaration) => {
                        if (T.isObjectProperty(declaration)) {
                            const name = getComponentName$1(declaration.key);
                            if (name != null) {
                                if (T.isIdentifier(declaration.value)) {
        ...
        case 'module':
            {
                ...
                if (path$.isImportDefaultSpecifier()) {
                    ...
                    return {
                        moduleName: parent.source.value,
                        exportName: T.isStringLiteral(node.imported)
                            ? node.imported.value
                            : node.imported.name,
                        localName: node.local.name,
                        loc: createSourceRange(context, parent),
...
function getTypeAnnotation(node$, context) {
    try {
        ...
        traverse__default['default'](node$.node, {
        ...
        const node = T.cloneNode(node$.isTSInterfaceDeclaration()
            ? node$.node.body
            : node$.node.typeAnnotation);
        const knownImports = new Set(imports.map((i) => i.localName));
        T.traverseFast(node, (node) => {
            if (T.isIdentifier(node) && !knownImports.has(node.name)) {
...
function stringifyBabelNode(node) {
    return generate__default['default'](node, {
        sourceMaps: false,
        concise: true,
    }).code;
}
            ...
            ExportNamedDeclaration(statement$) {
                if (context.mode !== 'setup') {
                    ...
                    if (statement.specifiers.length > 0) {
                        const specifier = statement.specifiers.find((specifier) => T.isExportSpecifier(specifier) &&
                    ...
                    else if (T.isTSTypeAliasDeclaration(statement.declaration) ||
...
function processInferredEmits(node$, context, inTemplate = false) {
    const { node, scope, parentPath } = node$ instanceof traverse.NodePath ? node$ : { node: node$ };
        ...
        return [
            {
                kind: 'expression',
                imports: [],
                expression: generate__default['default'](path$.node.typeAnnotation, { sourceMaps: false })
                    .code,
            },
        ];
    }
    ...
    setup: [
        (node$, context) => {
            ...
            if (setupContext != null) {
                let rest;
                const identifiers = {};
                if (T.isObjectPattern(setupContext)) {
                ...
                else if (T.isIdentifier(setupContext)) {
...
const TemplateBlockAnalyzer = createPlugin({
    blocks: {
        template: (block, ctx) => {
            if (block.src == null) {
                const ast = compilerTsx.parse(block.content, {
                    onError(error) {
                        if (error.loc != null) {
                ...
                templateAstTypes.traverse(ast, (node, ancestors) => {
                    if (templateAstTypes.isSimpleExpressionNode(node)) {
                        ...
                        if (templateAstTypes.isDirectiveNode(parent)) {
                            if (parent.exp === node) {
                                if (parent.name === 'for') {
                                    return;
                                }
...
function getPaths(dir) {
    ...
    const paths = [];
    while (dir !== Path__default['default'].posix.dirname(dir)) {
        paths.push(`${dir}/node_modules`);
        dir = Path__default['default'].posix.dirname(dir);
    }
...
function requireFileFromPackage(require, packageName, fileName, rootDir) {
    ...
    const req = Path__default['default'].posix.resolve(`/${packageName}`, fileName).substr(1);
        ...
        this.config.globalComponents.forEach((option) => {
            ...
            if (typeof option === 'string') {
                if (option in this.packageJSON.dependencies ||
                    ...
                    micromatch__default['default'].match(this._fileNames, option).forEach((fileName) => {
                        var _a;
                        this._globalComponents.push(...((_a = this._projectComponents.get(fileName)) !== null && _a !== void 0 ? _a : getComponentFromFile(fileName)));
                    });
                }
                else {
                    const fileName = Path__default['default'].posix.resolve(Path__default['default'].posix.dirname(this.configFile), option);
                ...
                Object.entries(option).forEach(([name, config]) => {
                    ...
                    const moduleName = id in this.packageJSON.dependencies ||
                        ...
                        ? id
                        : Path__default['default'].posix.resolve(Path__default['default'].posix.dirname(this.configFile), id);
...
function transformToFunction(content, options = {
    ...
    if (options.name == null) {
        const code = generate__default['default'](T.arrowFunctionExpression(params, T.blockStatement(statements))).code;
        ...
        return generate__default['default'](T.functionExpression(T.identifier(options.name), params, T.blockStatement(statements))).code.replace(/^function anonymous/, 'function ');
    }
}
...
exports.ComponentsOptionAnalyzer = ComponentsOptionAnalyzer;
exports.ConfiguredVueProject = ConfiguredVueProject;
exports.EmitsOptionsAnalyzer = EmitsOptionsAnalyzer;
exports.ImplicitEmitsAnalyzer = ImplicitEmitsAnalyzer;
exports.InferredVueProject = InferredVueProject;
exports.PropsOptionsAnalyzer = PropsOptionsAnalyzer;
exports.ScriptBlockAnalyzer = ScriptBlockAnalyzer;
exports.ScriptIdentifierSourceAnalyzer = ScriptIdentifierSourceAnalyzer;
exports.SetupOptionsAnalyzer = SetupOptionsAnalyzer;
exports.TemplateBlockAnalyzer = TemplateBlockAnalyzer;
exports.VueProject = VueProject;
exports.createAnalyzer = createAnalyzer;
exports.createFullAnalyzer = createFullAnalyzer;
exports.createSourceRange = createSourceRange;
exports.toVeturData = toVeturData;
exports.toWebTypes = toWebTypes;
exports.transformToFunction = transformToFunction;
