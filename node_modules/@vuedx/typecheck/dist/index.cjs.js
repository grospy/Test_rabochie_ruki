...
class TypeScriptServerHost {
    constructor() {
        ...
        this.serverPath = require.resolve('typescript/lib/tsserver');
        this.pluginPath = Path__default['default'].dirname(require.resolve('@vuedx/typescript-plugin-vue/package.json'));
        this.isClosed = false;
...
async function* getDiagnostics(directory, cancellationToken, logging = false) {
    ...
    function setDiagnostics(file, kind, diagnostics) {
        var _a;
        if (file.includes('/node_modules/'))
            return;
        if (diagnostics.length > 0) {
            const fileName = vueVirtualTextdocument.getContainingFile(file);
    ...
    const files = await glob__default['default'](['**/*.vue', '**/*.ts', '**/*.js', '**/*.jsx', '**/*.tsx'], {
        cwd: directory,
        absolute: true,
    ...
    const checkFile = (_a = files.find((file) => /\.(ts|js)x?/.test(file))) !== null && _a !== void 0 ? _a : files[0];
    ...
    if (((_b = project === null || project === void 0 ? void 0 : project.configFileName) === null || _b === void 0 ? void 0 : _b.endsWith('inferredProject1*')) === true) {
...
async function createTextDocument(file) {
    const content = await FSP__default['default'].readFile(file, { encoding: 'utf-8' });
    const fileName = toNormalizedPath(file);
    const document = TextDocument.create(fileName, Path__default['default'].posix.extname(file), 0, content);
...
async function formatDiagnosticsWithColorAndContext(fileName, diagnostic) {
    ...
    output += chalk__default['default'].gray(` ${(_a = diagnostic.source) !== null && _a !== void 0 ? _a : ''}${(_b = diagnostic.code) !== null && _b !== void 0 ? _b : ''}: `);
...
function convertToRelativePath(fileName) {
    return Path__default['default'].isAbsolute(fileName)
        ? Path__default['default'].relative(directory, fileName).replace(/\\/g, '/')
        : fileName;
}
async function cli() {
...
`.trim());
    ...
    directory =
        argv[0] != null
            ? Path__default['default'].isAbsolute(argv[0])
                ? argv[0]
                : Path__default['default'].resolve(process.cwd(), argv[0])
            : process.cwd();
...
exports.cli = cli;
exports.getDiagnostics2 = getDiagnostics2;
